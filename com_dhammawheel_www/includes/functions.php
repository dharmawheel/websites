...
function page_header($page_title = '', $display_online_list = false, $item_id = 0, $item = 'forum', $send_headers = true)
{
	...
	*/
	...
	// Check for dark mode cookie server-side
    $darkmode_cookie = $request->variable($config['cookie_name'] . '_darkmode', '', true, \phpbb\request\request_interface::COOKIE);
    if (!empty($darkmode_cookie)) {
        $user->set_cookie('darkmode', $darkmode_cookie, time() + 900 * 24 * 60 * 60 * 60);
    }
    $template->assign_var('IS_DARKMODE', $darkmode_cookie);
